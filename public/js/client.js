...
var peerChat=document.getElementById('peerChat');
let tabRoomSecurity;
// my video element
let myVideo;
let myVideoWrap;
...
function getHtmlElementsById() {
    ...
    peerDraggable = getId('peerDraggable');
    msgerHeader = getId('msgerHeader');
    peerHeader = getId('peerHeader');
...
function setButtonsToolTip() {
    ...
    setTippy(peerRoomBtn, 'See other clients in Room', 'right-start');
...
function handleButtonsRule() {
    ...
    elemDisplay(peerRoomBtn, buttons.main.showPeerRoomBtn);
...
async function whoAreYou() {
    ...
    Swal.fire({
        ...
        <br>
        <div style="overflow: hidden; vertical-align: left;">
            <button id="initGenMars" class="fas fa-mars" style="background:blue" onclick="handleGen( 'm')">
            
            </button><label style='font-size:12px;'>Man</label><br>
            <button id="initGenVen" class="fas fa-venus" style="background:red" onclick="handleGen( 'v')">
            
            </button><label style='font-size:12px;'>Woman</label><br>
            <button id="initGenTran" class="fas fa-transgender" style="background:orange" onclick="handleGen('t')">
            
            </button><label style='font-size:12px;'>Trans</label><br>
            <button id="initGenCoppia" class="fas fa-venus-mars" style="background:granata" onclick="handleGen('t')">
            
            </button><label style='font-size:12px;'>Couple</label><br>
...
async function handleAddPeer(config) {
    var appo=Object.keys(config.peers)
    for(var a=0;a<appo.length;a++){
        
        presencesPeers.push(config.peers[appo[a]]['peer_name'])
    }
    ...
    await msgerAddPeers(peers,config);
...
async function handleAddVideoPeer(config) {

    let oldWrapper=document.getElementById("remoteVideoContainer");
    let oldSlot=document.getElementById(config.peer_id+"_videoWrap");
    /*
    for(let a=0;a<oldWrapper.children.length;a++ ) {
        console.log("child.id  -  ",oldWrapper.children[a])
    }
    */
    
    for(let a=0;a<3;a++){
        let newWrapper=document.getElementById("mySlotWrap_"+a);
        if(!newWrapper.hasChildNodes()){
            oldWrapper.removeChild(oldSlot);
            newWrapper.appendChild(oldSlot);
            return
...
async function initEnumerateDevices() {
    ...
    if (!useAudio && !useVideo) {
        ...
        await loadWithoutMedia();
        for(let a=0;a<3;a++){
            await showVideoSlots(a);}
        /*
        openURL(
...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
async function showVideoSlots(a) {
    console.log('10. Access granted to audio - video device');
    // hide loading div
    getId('loadingDiv').style.display = 'none';
    ...
    // menu Status
    myStatusMenu.setAttribute('id', 'slot_'+a);
    myStatusMenu.className = 'statusMenu fadein';

    // session time
    //myCountTimeImg.setAttribute('id', 'countTimeImg_1');
    //myCountTimeImg.className = 'fas fa-clock';
    //myCountTime.setAttribute('id', 'countTime_');
    ...
    */
    ...
    //localMedia.setAttribute('id', 'Slot_');
    //localMedia.setAttribute('playsinline', true);
    //localMedia.className = 'mirror';
    //localMedia.autoplay = true;
    //localMedia.muted = true;
    ...
    videoWrap.className = 'Camera';
    videoWrap.setAttribute('id', 'mySlotWrap_'+a);
...
///////////////////////////////////////////////////////////////
async function loadWithoutMedia() {
    console.log('10. Access granted to audio - video device');
    // hide loading div
    getId('loadingDiv').style.display = 'none';
...
async function loadRemoteMediaStream(stream, peers, peer_id) {
    ...
    const closeRemoteCamera = document.createElement('button');
    ...
    // remote close Camera
    closeRemoteCamera.setAttribute('id', peer_id + '_closeCamera');
    closeRemoteCamera.className = 'fas fa-times';
    closeRemoteCamera.addEventListener('click',()=>{closeRemoteCam(peer_id)})
    ...
    remoteStatusMenu.appendChild(closeRemoteCamera);
    ...
    getId('remoteVideoContainer').appendChild(remoteVideoWrap);
...
function closeRemoteCam(id){
    
    let oldWrapper;
    let oldSlot=document.getElementById(id+"_videoWrap");
    let newWrapper=document.getElementById("remoteVideoContainer");
    for(let a=0;a<3;a++){
        oldWrapper=document.getElementById("mySlotWrap_"+a);
        console.log('ASD ASD    ',oldWrapper.childNodes[0].id)
        if(oldWrapper.childNodes[0].id==id+"_videoWrap"){
            oldWrapper.removeChild(oldSlot);
            newWrapper.appendChild(oldSlot);
            return
...
function setPeerRoomBtn() {
    // adapt chat room size for mobile
    setPeerRoomAndCaptionForMobile();

    // open hide chat room
    peerRoomBtn.addEventListener('click', (e) => {
        if (!isPeerRoomVisible) {
            showPeerRoomDraggable();
        } else {
            hidePeerRoomAndEmojiPicker();
    ...
    // close chat room - show left button and status menu if hide
    peerClose.addEventListener('click', (e) => {
        hidePeerRoomAndEmojiPicker();
        showButtonsBarAndMenu();
    });
    ...
    if (!isSupportedNavigatorShare || (isSupportedNavigatorShare && errorNavigatorShare)) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let message = {
                    email: '',
                    subject: 'Please join our Free Chat Video Meeting',
                    body: 'Click to join: ' + myRoomUrl,
                };
                shareRoomByEmail(message);
            }
 ...
 * Set the chat room on full screen mode for mobile
 */
function setPeerRoomAndCaptionForMobile() {
    if (isMobileDevice) {
        document.documentElement.style.setProperty('--msger-height', '99%');
        document.documentElement.style.setProperty('--msger-width', '99%');
    } else {
        // make chat room draggable for desktop
        dragElement(peerDraggable, peerHeader);
    }
}
...
function showChatRoomDraggable() {
    ...
    msgerDraggable.style.left = isMobileDevice ? '50%' : '70%';
...
function showPeerRoomDraggable() {
    playSound('newMessage');
    if (isMobileDevice) {
        buttonsBar.style.display = 'none';
        isButtonsVisible = false;
    }
    peerRoomBtn.className = 'fas fa-users';
    peerDraggable.style.top = '52%';
    peerDraggable.style.left = isMobileDevice ? '50%' : '70%';
    peerDraggable.style.display = 'flex';
    isPeerRoomVisible = true;

    // only for desktop
    if (!isMobileDevice) {
        setTippy(peerRoomBtn, 'Close the chat', 'right-start');
    }
}
...
function hidePeerRoomAndEmojiPicker() {
    peerDraggable.style.display = 'none';
    msgerEmojiPicker.style.display = 'none';
    msgerEmojiBtn.style.color = '#FFFFFF';
    peerRoomBtn.className = 'fas fa-users';
    isPeerRoomVisible = false;
    isChatEmojiVisible = false;
    // only for desktop
    if (!isMobileDevice) {
        setTippy(peerRoomBtn, 'Open the chat', 'right-start');
    }
}
...
function handleDataChannelChat(dataMessage) {
    ...
    sendToServer('checkMsg', {
        msgFrom:msgFrom,
        leftChatAvatar:leftChatAvatar,
        msg: msg,
...
function appendMessage(from, img, side, msg, privateMsg, msgId = null) {
	...
	<div class="msg ${side}-msg">
		...
		<div class=${msgBubble}>
            <div class="msg-info">
                <div class="msg-info-name" style="font-size:75%">${from}</div>
                <div class="msg-info-time" style="font-size:75%">${time}</div>
            </div>
            <div id="${chatMessagesId}" class="msg-text" style="font-size:75%">${msg}
                <!--hr/>
                <button
 ...
 * @param {object} peers all peers info connected to the same room
 */
async function msgerAddPeers(peers,config) {
        ...
        if (peer_id != myPeerId) {
            ...
            if (!exsistMsgerPrivateDiv) {
                ...
                */
                let peerDiv = `
                <div id="${peer_id}_pMsgDiv" class="msger-peer-inputarea">
                    <img id="${peer_id}_pMsgAvatar" src='${avatarApiUrl}?name=${peer_name}&size=24&background=random&rounded=true' style='background:black;'> 
                    <span style='background:black;padding-top:10px;'>${peer_name}</span>
                    <textarea
                        rows="1"
                        cols="1"
                        id="${peer_id}_pMsgInput"
                    ...
                    ></textarea>
                    <button id="${peer_id}_pMsgBtn" value="${peer_name}" style='background:black;'>
                        <i class="fas fa-paper-plane"></i>
                    </button>
                    <button id="${peer_id}_pVidBtn" value="${peer_name}" style='background:black;'>
                        <i class="fas fa-video"></i>
                    </button>
                </div>
                `;

                peerChat.insertAdjacentHTML('beforeend', peerDiv);
                var oncli=document.getElementById(peer_id+"_pVidBtn");
                
                oncli.addEventListener('click', (e) => {
                    // prevent refresh page
                    e.preventDefault();
                    handleAddVideoPeer(config);
                });
