!(function () {
    ...
    function shhow(){
        var n=document.getElementById('NationDropdown2')
    ...
    function creete(r){
        ...
        let div = document.getElementById("acco_container");
        for(let a=0;a<re.length;a++){
            ...
            <div class="c">
                <input type="checkbox" id="faq-'${a}'">
                    <h1>
                        <label for="faq-'${a}'">${re[a].name}</label>
                    </h1>
                <div class="p" id="p-${a}">
            ...
            `
           /*
            console.log(re[a].name)
            let div_c=document.createElement('div');
            div_c.className='c';

            let input_c=document.createElement('input');
            input_c.setAttribute('type','checkbox');
            input_c.setAttribute('id','faq-'+a);
            ...
            lab.setAttribute('for','faq-'+a);
            lab.createTextNode(re[a].name);


            h_1.appendChild(lab);
            input_c.appendChild(h_1);
            ...
            pp.createTextNode(re[a].sub);
            div_p.appendChild(pp);


            
            div_c.appendChild(input_c);
            div_c.appendChild(div_p)
            ...
            */
            ...
            //li.onclick=function(){testo(txt)}
        }
        for(let a=0;a<re.length;a++){
            let div_p = document.getElementById("p-"+a);
            for(let s=0;s<re[a].sub.length;s++){
                const lis= `
                <li style="list-style: none;" id="cl-${a}_${s}">${re[a].sub[s]}</li>
            `
            
            //console.log(re[a].sub)
            div_p.insertAdjacentHTML('beforeend',lis)
        ...
        for(let a=0;a<re.length;a++){
            let div_p = document.getElementById("p-"+a);
            for(let s=0;s<re[a].sub.length;s++){
                let elele=document.getElementById("cl-"+a+"_"+s)
                elele.addEventListener('click',(e)=>{
                    e.preventDefault();
                    startroom(re[a].name,re[a].sub[s])})
            }
    ...
    function testo(t){
        ...
        var raw = JSON.stringify({"name":t.textContent});
        var requestOptions = {
        ...
        fetch(window.location+"/listone", requestOptions)
                .then(response => response.text())
                .then(result => zone(result))
                .catch(error => console.log('error', error));

        let div = document.getElementById("fbut");
        div.innerText=t.textContent;
        document.getElementById("NationDropdown").classList.toggle("show");  
    }

    function filterFunction() {
        var input, filter, ul, li, a, i,txtValue;
        input = document.getElementById("natInput");
        filter = input.value.toUpperCase();
        div = document.getElementById("NationDropdown");
    ...
    function zone(r){
        ...
        for(let a=0;a<re.length;a++){
            ...
            let txt = document.createTextNode(re[a]);
            li.appendChild(txt);
            let div = document.getElementById("ZoneDropdown");
            div.appendChild(li);
            
            li.onclick=function(){red(txt)}
        }
    ...
    function nationList() {  
      ...
      function red(t){
        let div = document.getElementById("fbut");
        window.location.assign(window.location.href+"join/"+div.innerText+"_"+t.textContent)
      }
