...
const nlp_response = require("./nlp_mio");
...
const states=[
    {'name':'Comunità','sub':['fritto misto']},
    {'name':'Vita Reale',
     'sub':["Amici di CFV",
        "4 Chiacchiere",
        "Sport",
...
db.mongoose
  .connect(
    ...
    'mongodb+srv://guest:musica_260876@cfv-cluster.ifxmnjt.mongodb.net/test', {
        useNewUrlParser: true,
        useUnifiedTopology: true
        } 
    /*
    
    `mongodb://localhost:27017/FVC`, {
    useNewUrlParser: true,
    useUnifiedTopology: true
    }
    */
...
app.post(['/listone'],(req, res) => {
    db.data.findOne({name:req.body.name},(err,succ)=>{
        if(err){ 
            res.send(err)
        }else{
        ...
        if (!err && count === 0) {
            for(var a=0;a<states.length;a++){
                new Data({
                    ...
                    console.log("added 'nations' to Data collection");
                  });
...
io.sockets.on('connect', async (socket) => {
    ...
    socket.on('checkMsg', async (config) => {
        
        nlp_response.expo(config).then((msg) => {
           
            log.debug('!!!!!!!----- MIO  mio  -- ',msg)
          }).catch((msg) => {
            log.debug('err  -- ',msg)
          })
    });
...
(async () => {
    ...
    port: 3000,
    local_https: true,
    local_key:path.join(__dirname, '../ssl/key.pem'),
    local_cert:path.join(__dirname, '../ssl/cert.pem'),
    
    allow_invalid_cert: true,
    //host:'https://localtunnel.me',
    subdomain:'cfv-chat16'
  });
  ...
  console.log(`Server is running on host --  `+tunnel.url);
